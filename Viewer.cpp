...
FollowViewer::FollowViewer(glm::vec3 eye)
    : Viewer(eye)
{}

void FollowViewer::follow(glm::vec3 focus, float height, float zoom)
{
  glm::vec3 up(0.0f, 1.0f, 0.0f);
  glm::vec3 from;
  from.x = (zoom/7) * focus.x;
  from.y = (zoom/7) * focus.y + ((7-zoom)/7) * height;
  from.z = (zoom/7) * focus.z;
  viewMtx = glm::lookAt(from, focus, up);
}
